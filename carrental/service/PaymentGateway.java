...
public class PaymentGateway {
    ...
    public Payment charge(String reservationId, double amount) {
        String paymentId = "PAY-" + UUID.randomUUID().toString().substring(0, 8);
        PaymentStatus status = random.nextDouble() < successRate
            ? PaymentStatus.COMPLETED : PaymentStatus.FAILED;
        return new Payment(paymentId, reservationId, amount, status,
    ...
    public Payment chargeAdditional(String reservationId, double amount) {
        return charge(reservationId, amount);
    }
}
